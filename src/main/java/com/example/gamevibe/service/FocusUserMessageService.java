package com.example.gamevibe.service;


import com.baomidou.mybatisplus.extension.service.IService;
import com.example.gamevibe.model.entity.FocusUserMessage;
import com.example.gamevibe.model.vo.FocusUserMessageVO;

import java.util.List;

/**
* @author ZML
* @description 针对表【focus_user_message(关注用户消息表)】的数据库操作Service
* @createDate 2024-06-23 11:38:51
*/
public interface FocusUserMessageService extends IService<FocusUserMessage> {

    List<FocusUserMessageVO> listFocusUserMessage();
}
