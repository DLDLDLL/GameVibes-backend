...
public class PostCollectController {
    ...
    public BaseResponse<PageVO<MyPostCollectVO>> listPostCollectVOByPage(@RequestBody(required = false) @Validated PageRequest pageRequest, @RequestParam(required = false) String user_id) {
    ...
    @ApiResponse(code = 0, message = "ok")
    @PostMapping("/collect")
    public BaseResponse<String> collect(@RequestParam("post_id") @NotNull(message = "帖子id不能为空") Long post_id) {
    ...
    public BaseResponse<String> unCollect(@RequestParam("post_id") @NotNull(message = "帖子id不能为空") Long post_id) {
