...
public interface PostCollectMapper extends BaseMapper<PostCollect> {
    ...
    @Select("SELECT p.id, u.user_id, u.nick_name, u.avatar, p.post_time, p.title, p.content, p.images, p.comments, pl.like_count, pc.collect_count, pl.is_like, 1 AS is_collect " +
            "FROM (SELECT post_id FROM post_collect WHERE user_id = #{user_id} AND state = 1) pcu " +
            "LEFT JOIN post p ON pcu.post_id = p.id " +
